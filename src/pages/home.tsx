...
const Home = () => {
  ...
  const coverMessage =
    "Game like a pro. Dominate every match with cutting-edge gear built for champions. Your setup defines your victory — power up, play hard, and leave the competition behind.".split(
      " "
    );
  return (
    <>
            ...
            <>
              {Array.from({ length: 6 }, (_, i) => (
                <div key={i} style={{ height: "25rem" }}>
                  <SkeletonLoader width="18.75rem" count={1} height="20rem" />
                  <SkeletonLoader width="18.75rem" count={2} height="1.95rem" />
                </div>
              ))}
            </>
          ) : (
